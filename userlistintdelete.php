...
if (isset($_GET['id'])) {
    ...
    try {
        ...
        $query1 = "DELETE FROM interviewer_information WHERE user_id = '$id'";
        if (!mysqli_query($conn, $query1)) {
            throw new Exception('Error deleting from interviewer_information: ' . mysqli_error($conn));
        }
        ...
        header("Location: userlistint.php?success=" . urlencode($successMessage));
        exit();
    } catch (Exception $e) {
        ...
        if (strpos($e->getMessage(), 'Cannot delete or update a parent row') !== false) {
            $errorMessage = 'Error: You need to delete related records first.';
        } else {
            $errorMessage = $e->getMessage();
        }
        
        // Redirect back to the main page with error message
        header('Location: userlistint.php?error=' . urlencode($errorMessage));
        exit();
    }
} else {
    // If the ID parameter is not present, redirect back to the main page with error message
    header('Location: userlistint.php?error=ID parameter not specified.');
    exit();
}
